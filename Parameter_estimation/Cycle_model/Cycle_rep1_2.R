...
cm <- read.table("../Data/Cycle_random_initial.txt",sep="\t",skip=1)
cm <- cbind(cm[,2:5],cm[,7:8])
cm <- cm[21:40,]
cm <- as.data.frame(cm)
...
for(i in 1:dim(cm)[1]){
  ...
  result <- c()
  result <- sbplx(c(pa1,pa2,pa3,pa4,pa5,pa6),h,control=list(maxeval=200),lower=c(k1=6e-05,k2=log(2)/100,KD1=0.001,KD2=0.001,kdeg=2.000000e-05,tau=0),upper=c(k1=6e-03,k2=log(2)/0.01,KD1=1000,KD2=1000,kdeg=7.701635e-04,tau=7200))
...
write.table(ps, "Cycle_gene.name_rep1_2.txt", sep = "\t")
