...
inputkd <- sigimp
  ...
  tpm <- as.data.frame(tpm)
  rownames(tpm) <- rownames(count) #‚¢‚Â‚à‚Ì‚Írownames(count1)Alongesttranscripts‚Írownames(data)
  colnames(tpm) <- seq(0,180,15)
  kd <- tpm
  ...
  g <- function(kdeg,KD1,KD2,tau){
   ...
   rmsdkd <- rmsd(refkdfc[,2],ekd(kdeg,KD1,KD2,tau)[which(ekd(kdeg,KD1,KD2,tau)[,1] %in% seq(0,10800,900)),2],fit=FALSE)
    mul <- rmsdkd
  ...
  cm <- read.table("../First_step_optimization/IFFL_model/Bounds/IFFL_gene.name_Subplex_random_initial.txt",sep="\t",skip=1)
  cm <- cm[1:50,2:5]
  cm <- as.data.frame(cm)
  
  
  data <- read.table("../First_step_optimization/IFFL_model/Optimized_parameters/IFFL_gene.name_subplex_rep1.txt",sep="",skip=1)
  ...
  data <- read.table("../First_step_optimization/IFFL_model/Optimized_parameters/IFFL_gene.name_subplex_rep2.txt",sep="",skip=1)
  data <- as.data.frame(data)
  data <- data[which(data[,2]<0.5),]
  ...
  for(i in 1:dim(cm)[1]){
    ...
    result <- c()
    result <- sbplx(c(pa1,pa2,pa3,pa4),h,control=list(maxeval=200),lower=c(kdeg=kdegmin,KD1=KD1min,KD2=KD2min,tau=taumin),upper=c(kdeg=kdegmax,KD1=KD1max,KD2=KD2max,tau=taumax))
  ...
  write.table(ps, "2_IFFL_gene.name_rep1_1.txt", sep = "\t")
