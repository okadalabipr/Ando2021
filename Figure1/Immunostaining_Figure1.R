library(ggplot2)
    ...
    g <- ggplot(wtsignal,aes(x=time,y=nfkb,color=group,size=group))+
      ...
      scale_y_continuous(breaks=seq(0.3,0.6,0.05),labels=waiver(),limits=c(0.3,0.6))+
      geom_point(aes(shape=group,color=group))+
      scale_color_manual(values=c(none="#DC143C",atac="black"))+
