...
#df[which(df$name=="IRS2"),2]="yellow"
#df[which(df$name=="JUNB"),2]="yellow"
#df[which(df$name=="KLF10"),2]="yellow"
#df[which(df$name=="MAP3K8"),2]="yellow"
#df[which(df$name=="NUAK2"),2]="yellow"
#df[which(df$name=="PHLDA1"),2]="yellow"
#df[which(df$name=="PPP1R15A"),2]="yellow"
...
except_genes <- c("ABAT","DAXX","FAM117A","FGD6","HMGCR","IER5L","IFNGR2","IL6ST","IRF2","ITGB8","LINC00052","NFKB1","OPTN","PHF23","PRKAR2B","RFX5","SEMA4B","SLC6A14","STAT5A","TBC1D9","TNFAIP2","VDR")
...
#count <- read.csv("../Data/160617_ikbakd.csv")#ファイル読み込み(データフレーム形式)
#count1=count[,2:12]
#count1 <- data.frame(count1)
#mean <- count1[,2]
#condition <- count1[,1]
